...
describe AthenaEvent do
  ...
  describe "performances" do
    ...
    it "should fetch the performances from ATHENA if not yet cached" do
      ...
      subject.id = 1
      FakeWeb.register_uri(:get, 'http://localhost/athena/performances.json?eventId=eq1', :status => 200, :body => test_performances.to_json)
  ...
  describe "charts" do
    ...
    it "should fetch the charts from ATHENA if not yet cached" do
      charts = (0..5).collect { Factory(:athena_chart) }
      subject.id = 1
      FakeWeb.register_uri(:get, 'http://localhost/athena/charts.json?eventId=eq1', :status => 200, :body => charts.to_json({}))
      subject.charts.size.should eq charts.size
    end
  ...
  describe "#as_widget_json" do
    ...
    it "should not include performances that are on sale" do
      ...
      subject.performances.first.state = "published"
