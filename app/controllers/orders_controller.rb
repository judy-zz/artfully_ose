class OrdersController < ApplicationController
  def index
    ...
    if params[:search]
      ...
      @results = AthenaOrder.find(:all, :params =>{ :organizationId => "eq#{current_user.current_organization.id}"}).sort{|a,b| b.timestamp <=> a.timestamp }.paginate(:page => params[:page], :per_page => 25)
    end
  end
