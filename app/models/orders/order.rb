...
class Order < ActiveRecord::Base
  ...
  def self.sale_search(search)        
    ...
    standard = ::Order.where("type != ?", Reseller::Order.name).includes(:items => { :show => :event })
    reseller = Reseller::Order.includes(:items => { :show => :event })

    if search.start   
      standard = standard.after(search.start)
      reseller = reseller.after(search.start)
    end

    if search.stop   
      standard = standard.before(search.stop)
      reseller = reseller.before(search.stop)
    end

    if search.organization
      standard = standard.where('orders.organization_id = ?', search.organization.id)
      reseller = reseller.where('orders.organization_id = ?', search.organization.id)
    end

    if search.show
      standard = standard.where("shows.id = ?", search.show.id)
      reseller = reseller.where("shows.id = ?", search.show.id)
    elsif search.event
      standard = standard.where("events.id = ?", search.event.id)
      reseller = reseller.where("events.id = ?", search.event.id)
    end

    standard.all + reseller.all
  end
