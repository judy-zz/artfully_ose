class User < ActiveRecord::Base
  ...
  def self.like(query = "")
    ...
    q = "%#{query}%"
    self.joins("LEFT OUTER JOIN memberships ON memberships.user_id = users.id").joins("LEFT OUTER JOIN organizations ON organizations.id = memberships.organization_id").where("users.email like ? or organizations.name like ?", q, q)
  end
