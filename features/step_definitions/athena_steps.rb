...
Given /^I can save Credit Cards to ATHENA$/ do
  FakeWeb.register_uri(:post, "http://localhost/athena/payments/cards.json", :status => 200)
end

Given /^I can authorize Credit Cards in ATHENA$/ do
  FakeWeb.register_uri(:post, "http://localhost/athena/payments/transactions/authorize", :body => "{ success:true, transaction_id:'j59qrb' }")
end
...
Given /^I can save Customers to ATHENA$/ do
  FakeWeb.register_uri(:post, "http://localhost/athena/payments/customers.json", :body => Factory(:customer_with_id).encode)
end
