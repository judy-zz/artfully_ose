Feature: View the record for a person
  ...
  Scenario: A user attempts to view a Person record for a non-existent user
    ...
    And I am on the people search page
    When I fill in "search" with "doesnotexist@example.com"
    And I press "Search"

  Scenario: A user searches for the Person Record for another user
    Given I am logged in
    And I am part of an organization
    And a person exists with an email of "person@example.com" for my organization
    And "person@example.com" is in the index
    And I am on the people search page
